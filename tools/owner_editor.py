...
def _is_sqlite_mode():
    """Sprawdza czy aplikacja działa w trybie SQLite."""
    ...
    if SQLITE_MODE:
        ...
        try:
            ...
            if result:
                location_name = result[0]
                backup_folder = os.path.join(base_dir, "data", "locations", location_name)
                print(f"✅ Edytor właścicieli (PostgreSQL) - aktywna miejscowość: {location_name}")
                return backup_folder
        except Exception as e:
            print(f"⚠️ PostgreSQL niedostępny: {e}")

    print(f"⚠️ Używam domyślnej lokalizacji")
    return os.path.join(base_dir, "data", "locations")
