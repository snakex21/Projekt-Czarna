...
def test_backup_folder_exists():
    ...
    print(f"\n[TWIERDZA] Katalog backup znaleziony: {backup_path}")
...
def test_location_data_files_present():
    ...
    loc_path = os.path.join(BASE_DIR, "backup", location)
    ...
    print(f"\n[TWIERDZA] Wszystkie wymagane pliki dla {location} są na miejscu.")
...
def test_backup_file_integrity():
    ...
    if size < 100: # Za mały na sensowne dane
        print(f"\n[TWIERDZA] ⚠️ OSTRZEŻENIE: Plik genealogia.json jest podejrzanie mały ({size} bajtów).")
    ...
    try:
        with open(path, 'r', encoding='utf-8') as f:
            ...
            print(f"[TWIERDZA] Struktura JSON poprawna. Liczba osób: {len(data.get('persons', []))}")
...
def test_recent_modification():
    ...
    if diff_days > 30:
        print(f"\n[TWIERDZA] ⚠️ OSTRZEŻENIE: Dane nie były zmieniane od {int(diff_days)} dni.")
    else:
        print(f"\n[TWIERDZA] Dane są aktualne (ostatnia zmiana {int(diff_days)} dni temu).")
    assert True
