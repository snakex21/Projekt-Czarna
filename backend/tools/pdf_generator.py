...
BACKUP_DIR = os.path.join(BASE_DIR, "backup", "Czarna") # Domyślna lokalizacja, w przyszłości dynamiczna
...
def generate_family_pdf(person, all_persons=None, output_buffer=None):
    ...
    # 2. Znalezienie właściciela
    house_num = person.get("houseNumber")
    owner_info = find_owner_by_house(house_num, owners_db)
    ...
    with PdfPages(output_buffer) as pdf:
        ...
        for key, parcel_data in parcels_geo.items():
            ...
            if geo and len(geo) > 0:
                if isinstance(geo[0], list) and isinstance(geo[0][0], (float, int)):
                     coords = [(pt[1], pt[0]) for pt in geo]
                elif isinstance(geo[0], list) and isinstance(geo[0][0], list):
                    coords = [(pt[1], pt[0]) for pt in geo[0]]
        ...
        if all_x and all_y:
            ...
            for key, parcel_data in parcels_geo.items():
                ...
                geo = parcel_data.get("geometria")
                coords = []
                if geo and isinstance(geo[0], list):
                     coords = [(pt[1], pt[0]) for pt in (geo[0] if isinstance(geo[0][0], list) else geo)]
