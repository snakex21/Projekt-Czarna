...
def test_historical_photos_exist():
    ...
    if not os.path.exists(photos_dir):
        print(f"\n🖼️ Katalog zdjęć nie istnieje: {photos_dir}")
    ...
    print(f"\n🖼️ Znaleziono {len(files)} zdjęć historycznych w katalogu {location}.")
    assert True
...
def test_custom_icons_existence():
    ...
    if not os.path.exists(icons_dir):
        print("\n🖼️ ⚠️ Brak katalogu ikon!")
    else:
        print(f"\n🖼️ Katalog ikon dostępny ({len(os.listdir(icons_dir))} plików).")
    assert True
...
def test_genealogy_protocol_links():
    ...
    missing_protocols = []
    protokoly_dir = os.path.join(BASE_DIR, "data", "locations", location, "protokoly")
    
    if not os.path.exists(protokoly_dir):
        print(f"\n🖼️ ⚠️ Katalog protokołów nie istnieje: {protokoly_dir}")
        # Nie możemy zweryfikować linków bez katalogu protokołów
        # To nie jest błąd - protokoły mogą być przechowywane gdzie indziej
        return
    ...
    if missing_protocols:
        print(f"\n🖼️ Brakujące odniesienia do protokołów ({len(missing_protocols)}):")
        for m in missing_protocols[:20]:
            print(f"  ? {m}")
    else:
        print("\n🖼️ Wszystkie odniesienia do protokołów są poprawne.")
    assert True
